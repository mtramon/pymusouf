...
@dataclass(slots=True)
class Timestamp:
    """Acquisition timestamp with sub-second timing stored in integer 10 ns ticks."""

    s: int  # sec
    ns: int  # 10 ns ticks; per-PM raw value or per-event mean depending on context
    res: float = field(default=1e-8)  # explicite: 10 ns resolution
...
class ImpactPM:
    ...
    def readline(self):
        ...
        try:
            ...
            raise ValueError(f"Non-integer timestamp field: {self.line}") from exc
        if ts_s < 1e9:
            raise ValueError(f"Inconsistent timestamp: {self.line}")
        self.evt_id = int(fields[1])
        self.pm_id = int(fields[5])
        self.nhits = int(fields[8])
        ...
        hits = fields[9:expected_len]
        # self._channel_no = list(map(int, hits[0::2]))
        # self._adc = list(map(float, hits[1::2]))
        self._hits_raw = [
            (int(ch), float(adc)) for ch, adc in zip(hits[0::2], hits[1::2])
        ]
...
def convert_bar_to_mm(
    xyz: np.ndarray, scint_width: np.ndarray, z_mm: np.ndarray, norm_factor=None
) -> np.ndarray:
...
def apply_jitter(hits, scale=0.5):
    jitter = np.random.uniform(-scale, scale, size=hits.shape)
...
class Event:
    ...
    _nanotime_sum: int = field(init=False, repr=False)
    _nanotime_count: int = field(init=False, repr=False)
    _timestamp_counts: Dict[int, int] = field(init=False, repr=False)

    def __post_init__(self):
        ts_s = int(self.timestamp.s)
        self._nanotime_sum = int(self.timestamp.ns)
        self._nanotime_count = 1
        self._timestamp_counts = {ts_s: 1}

    def observe_timestamp(self, timestamp: Timestamp) -> None:
        ts_s = int(timestamp.s)
        self._timestamp_counts[ts_s] = self._timestamp_counts.get(ts_s, 0) + 1
        self._nanotime_sum += int(timestamp.ns)
        self._nanotime_count += 1

    def finalize_timestamp(self) -> None:
        if self._nanotime_count < 1:
            raise ValueError(f"Missing timestamp samples for event {self.id}")
        max_count = max(self._timestamp_counts.values())
        mean_second = min(
            second
            for second, count in self._timestamp_counts.items()
            if count == max_count
        )
        mean_nanotime = int(np.floor((self._nanotime_sum / self._nanotime_count) + 0.5))
        self.timestamp = Timestamp(s=mean_second, ns=mean_nanotime)
    ...
    def _xyz_bar(self, dict_ixpos) -> np.ndarray:
        ...
        for _, imp in self.impacts.items():  # loop on impacts (=impacted panels)
            ...
            hits_x = [
                h for h in imp.hits if isinstance(h.bar_no, str) and h.bar_no[0] == "X"
            ]
            hits_y = [
                h for h in imp.hits if isinstance(h.bar_no, str) and h.bar_no[0] == "Y"
            ]
    ...
    def get_xyz(
        self, dict_ixpos: dict, mm: bool = False, scint_width=None, z_mm=None
    ) -> None:
        """ """
        ...
        if len(self.xyz) == 0:
            return
        if mm:
            self.xyz = convert_bar_to_mm(self.xyz, scint_width, z_mm)
    ...
    def fit(self, hits):
        ...
        for h1, h2 in product(hits_min, hits_max):
            ...
            for z in planes:
                ...
                dists = np.linalg.norm(np.cross(plane_hits[:, :3] - p0, v), axis=1)
                imin = np.argmin(dists)
                dmin = dists[imin]
...
class Tracking:
    def __init__(self, telescope: Telescope, tan_max: float = 1.0):
        self.tel = telescope
        self.tan_max = tan_max
        self.panels = {p.id: p for p in self.tel.panels}
        self.dict_id_zpos = {id: p.position.z for id, p in self.panels.items()}
        self.dict_ixpos_zpos = {
            p.position.index: p.position.z for id, p in self.panels.items()
        }
        self.zpos = list(self.dict_id_zpos.values())
        self.dict_id_ixpos = {id: p.position.index for id, p in self.panels.items()}
        self.dict_cfg_zpos = {
            cfg_name: [p.position.z for p in cfg.panels]
            for cfg_name, cfg in self.tel.configurations.items()
        }
        self.dict_cfg_ix = {
            cfg_name: [p.position.index for p in cfg.panels]
            for cfg_name, cfg in self.tel.configurations.items()
        }
        self.dict_ixpos_width = {
            p.position.index: float(p.matrix.scintillator.width)
            for _, p in self.panels.items()
        }
        self.arr_scint_width = np.array(
            [self.dict_ixpos_width[i] for i in range(len(self.dict_ixpos_width))]
        )
        self.arr_z_mm = np.array(
            [self.dict_ixpos_zpos[i] for i in range(len(self.dict_ixpos_zpos))]
        )
        self.last_evt_id = None
        self.pitch_xy = np.array(
            [p.matrix.scintillator.width for _, p in self.panels.items()]
        )
...
class RansacTracking(Tracking):
    ...
    def main(self, file_out=None, chunks=None, n_jobs=None, **kwargs_model):
        ...
        dicts = {"ixpos": self.dict_id_ixpos, "zpos": self.dict_id_zpos}
        thresh = kwargs_model["residual_threshold"]
        nx, ny = [], []
        for p in self.tel.panels:
            nx.append(p.matrix.nbars_x)
            ny.append(p.matrix.nbars_y)
        det_config = dict(nx=nx, ny=ny, residual_threshold=thresh)
        ransac_config = kwargs_model
        models_config = {"det": det_config, "ransac": ransac_config}

        if chunks is None:
            raise ValueError(
                "Event chunks generator is required for RansacTracking.main()"
            )
        ...
        pd.DataFrame(columns=columns).to_csv(file_out, index=False, compression="gzip")
        # ----------------------------------
        # EVENT DISPLAY
        # ----------------------------------
        ...
        for rows, plots, ntot, nprocess, ntrack in results_tracking:
            ...
            if rows:
                ...
                df.to_csv(
                    file_out, mode="a", header=False, index=False, compression="gzip"
                )
            # display
            if len(plots) > 0:
                for (
    ...
    def _process_event(self, evt: Event, ndisplays=int):
        ...
        hits_by_plane = {int(z): hits_ev[hits_ev[:, 2] == z] for z in z_ev}
        # ----------------------------------
        ...
        for cfg_name, ixpos_cfg in self.dict_cfg_ix.items():
            ...
            if self.adc_ref is not None:
                ...
                p0_weight, v_weight = weighted_line_fit_2d(
                    xyz_inliers_mm,
                    adc_inliers,
                    mpv_ref=np.column_stack((mpv_x, mpv_y)),
                    sigma_ref=np.column_stack((sigma_x, sigma_y)),
                )
            ...
            if ndisplays > 0:
                ...
                if self.adc_ref is not None:
                    xyz_bary, weights_cfg_x, weights_cfg_y = barycenter_per_plane_2d(
                        xyz_cfg, adc_cfg, self.adc_ref
                    )
                    xyz_bary_mm = convert_bar_to_mm(
                        xyz_bary, self.arr_scint_width, self.arr_z_mm
                    )
                plots.append(
                    (
                        evt.id,
...
def save_event_2d(event_id, config_data, output_dir, z_panels=None, adc_ref=None):
    ...
    color_map = {"3p1": "blue", "3p2": "green", "4p": "red"}
    ax_xz, ax_yz = axes
    ...
    for i, (
    ...
    ) in enumerate(config_data.items()):
        adc_x = adc[:, 0]
        adc_y = adc[:, 1]
        print(
            f"Plotting event {event_id} config {cfg} with {len(inliers_idx)} inliers (RMS: ({rms_ransac:.2f}, {rms_weight:.2f}), MIP: ({mip_score_x:.2f}, {mip_score_y:.2f})) "
        )
        ...
        if i == 0:
            ax_xz.scatter(
                ...
                edgecolor="black",
                linewidth=0.5,
                label="Hits with weights" if weights_cfg_x is not None else "Hits",
                sizes=sizes_xz,
            )
        ...
        ax_xz.scatter(
            xyz_cfg_mm[mask_inliers][:, 0],
            xyz_cfg_mm[mask_inliers][:, 2],
            s=sizes_xz[mask_inliers],
            alpha=0.6,
            color=color,
        )
        ...
        ax_xz.plot(
            ...
            z_ransac,
            linewidth=2,
            label=f"{cfg} track ransac RMS_norm:{rms_ransac:.2f}",
            color=color,
            linestyle="dashed",
        )
        ax_xz.plot(
            ...
            z_fin,
            linewidth=2,
            label=f"{cfg} track final RMS_mm:{rms_weight:.2f}, MIP:{mip_score_x:.2f}",
            color=color,
        )
        # -------------------
        # YZ
        ...
        if i == 0:
            ax_yz.scatter(
                ...
                edgecolor="black",
                linewidth=0.5,
                label="Hits with weights" if weights_cfg_y is not None else "Hits",
                sizes=sizes_yz,
            )
        ...
        ax_yz.scatter(
            xyz_cfg_mm[mask_inliers][:, 1],
            xyz_cfg_mm[mask_inliers][:, 2],
            s=sizes_yz[mask_inliers],
            alpha=0.6,
            color=color,
        )
        ...
        ax_yz.plot(
            ...
            z_fin,
            linewidth=2,
            label=f"{cfg} track final RMS_mm:{rms_weight:.2f}, MIP:{mip_score_y:.2f}",
            color=color,
        )
