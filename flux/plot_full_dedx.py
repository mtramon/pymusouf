...
if __name__ == "__main__":
    ...
    for medium in [Rock, Water]:
        ...
        for rho, col in zip(l_rho, l_col) :      
            ...
            ax.plot(x, dEdx_ion, color=col,  label="ionization", linestyle="-.")
            ax.plot(x, dEdx_rad, color=col, label="radiative", linestyle="--")
    ...
    handle_ion = mlines.Line2D([], [], color='black',  linestyle='-.',
                            markersize=10, label="ionization")
    handle_rad = mlines.Line2D([], [], color='black',  linestyle='--',
